...
namespace BizTalk_Port_Multiplier
{
    public class XmlHelper
    {
        ...
        internal void RemoveExcessBindings(string originalPath, string path, bool receive, string portToKeep, string replaceName, string replacePort)
        {   //it's like lipossuction, but for a fat XML, and where we replace stuff
            try
            {
                XmlDocument bindings = new XmlDocument();
                ...
                {   //a little trim here
                    ...
                    foreach (XmlNode receivePort in bindings.SelectNodes("//BindingInfo//ReceivePortCollection//ReceivePort"))
                    {
                        if (receivePort.Attributes["Name"].Value != portToKeep)
                            receivePort.ParentNode.RemoveChild(receivePort);
                        else
                        if (!string.IsNullOrEmpty(replaceName))
                        {
                            receivePort.Attributes["Name"].Value = replaceName;
                            //receivePort.SelectSingleNode("ReceiveLocations//Address").InnerText = replacePort;
                        }
                        foreach (XmlNode child in receivePort.SelectSingleNode("//ReceiveLocations"))
                        {
                            child.Attributes["Name"].Value = child.Attributes["Name"].Value + "_" + DateTime.Now.ToString("yyyyMMddHHmmss");
                            child["Address"].InnerText = child["Address"].InnerText + "_" + DateTime.Now.ToString("yyyyMMddHHmmss");
                        }
                    }
                }
                else
                {   //a little trim there
                    ...
                    foreach (XmlNode sendPort in bindings.SelectNodes("//BindingInfo//SendPortCollection//SendPort"))
                    {
                        if (sendPort.Attributes["Name"].Value != portToKeep)
                            sendPort.ParentNode.RemoveChild(sendPort);
                        else
                            if (!string.IsNullOrEmpty(replaceName))
                        {
                            sendPort.Attributes["Name"].Value = replaceName;
                            sendPort.SelectSingleNode("//PrimaryTransport//Address").InnerText = replacePort;
                        }
                    }
                }
                bindings.Save(path);
